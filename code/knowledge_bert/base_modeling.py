...
class BertAttention(nn.Module):
    def __init__(self, config):
        ...
        config_ent.hidden_size = 200
        config_ent.num_attention_heads = 8
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(200, config.intermediate_size)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        self.dense_ent = nn.Linear(config.intermediate_size, 200)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.LayerNorm_ent = BertLayerNorm(200, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertBaseLayer_simple(nn.Module):
    def __init__(self, config):
        super(BertBaseLayer_simple, self).__init__()
        self.attention = BertAttention_simple(config)
        self.intermediate = BertIntermediate_simple(config)
        self.output = BertOutput_simple(config)
...
class BertBaseEncoder(nn.Module):
    def __init__(self, config):
        ...
        layer_base = BertBaseLayer_simple(config)
        layers = []
        for t in config.layer_types:
            layers.append(copy.deepcopy(layer_base))
        for _ in range(config.num_hidden_layers-len(layers)):
            layers.append(copy.deepcopy(layer_base))
        self.layer = nn.ModuleList(layers)
...
class BertBaseModel(PreTrainedBertModel):
    def __init__(self, config):
        super(BertBaseModel, self).__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertBaseEncoder(config)
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)
...
class BertBaseForEntityTyping(PreTrainedBertModel):
    def __init__(self, config, num_labels=2):
        super(BertBaseForEntityTyping, self).__init__(config)
        self.num_labels = num_labels
...
class BertBaseForSequenceClassification(PreTrainedBertModel):
    def __init__(self, config, num_labels=2):
